...
void HashTable::Insert(Bid bid) {
    ...
    if (previous == nullptr) {
        ...
        Node* next = new Node(bid, key); 
        nodes.insert(nodes.end()+ key, (*next));
    }
...
void HashTable::Remove(string bidId) {
    ...
    unsigned removalKey = hash(atoi(bidId.c_str()));
    nodes.erase(nodes.begin() + removalKey);
}

Bid HashTable::Search(string bidId) {
    Bid bid;
    unsigned searchKey = hash(atoi(bidId.c_str()));
    Node* previous = &(nodes.at(searchKey));
    // FIXME (8): Implement logic to search for and return a bid
    for (unsigned int i = 0; i < nodes.size(); i++) {
        if (nodes.at(i).bid.bidId.compare(previous->bid.bidId)) {
            return bid;
        }
    }
    // create the key for the given bid
